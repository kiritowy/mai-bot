...
# 查看当前群分管
gad = on_command('bot听谁话', aliases={"/gad", "/分群管理"}, priority=1, block=True,
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    rely = str(admins[gid])
    await gad.send(f"在这个群里我听他们话：{rely}")
...
# 添加分群管理员
g_admin = on_command('听他话', aliases={"/gad+", "分群管理+"}, priority=1, block=True,
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if sb and "all" not in sb:
        ...
        sb = str(event.get_message()).split(" ")
        for qq in sb:
            g_admin_handle = await approve.gadmin_add(gid, int(qq))
            if g_admin_handle:
                await g_admin.send(f"{qq}已成为本群分群管理：将接收加群处理结果")
...
# 删除分群管理
g_admin_ = on_command('不听他话', aliases={"/gad-", "分群管理-"}, priority=1, block=True,
                      permission=SUPERUSER)
    ...
    async for sleeped in baning:
        if sleeped:
            try:
                ...
                await sleep.finish("呜呜呜你欺负我没有权限烟你")
            else:
                await sleep.finish(f"你的{time/60}分钟精致睡眠套餐来啦")
                logger.info("禁言操作成功")
                

ban = on_command('点烟', priority=1, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if sb:
        if len(msg.split()) == len(sb) + 1:
            # logger(msg)
            time1 = int(msg.split()[-1:][0])
            # logger(time)
            baning = banSb(gid, ban_list=sb, time=time1)
            async for baned in baning:
                if baned:
                    try:
                        await baned
                    except ActionFailed:
                        await ban.finish("呜呜呜没有权限")
                    else:
                        logger.info("禁言操作成功")
        else:
...
unban = on_command("灭烟", priority=1, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER)
...
change = on_command('改马甲', permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=1, block=True)
...
title = on_command('改头衔', permission=SUPERUSER | GROUP_OWNER, priority=1, block=True)
...
kick = on_command('/飞机票', permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=1, block=True)
