...
from src.libraries.enet_b50_list import *
...
class ChartInfo(object):
    ...
    def from_json(cls, data):
        ...
        return cls(
            ...
            ra=computeRa(ds=data['ds'], achievement=data['achievements']),
            ds=data["ds"],
            comboId=fi,
            scoreId=ri,
            lv=data["level"],
...
class DrawBest(object):
    ...
    def __init__(self, sdBest:BestList, dxBest:BestList, userName:str):
        ...
        self.pic_dir = 'src/static/mai/pic/'
        self.cover_dir = 'src/static/mai/cover2/'
    ...
    def draw(self):
        ...
        authorBoardDraw.text((31, 28), '     Shabi\nFnmdp & Nmsl', 'black', font2)
...
def enetgenerate50(payload: Dict) -> Tuple[Optional[Image.Image], bool]:
    ...
    #         return None, 403
        ...
        dx_best = BestList(15)
        # obj = await resp.json()
        obj = getb50list(payload)
        obj1 = json.loads(obj)
        print(obj1[0]['dx'])
        dx: List[Dict] = obj1[0]["dx"]
        sd: List[Dict] = obj1[0]["sd"]
        for c in sd:
            sd_best.push(ChartInfo.from_json(c))
        for c in dx:
            dx_best.push(ChartInfo.from_json(c))
        pic = DrawBest(sd_best, dx_best, obj1[0]["nickname"]).getDir()
        print(pic)
        return pic, 0
